...
5. **Campaña = \`campaignName\`** (el utmCampaign de Meta) en la oportunidad, con caída a la atribución del contacto. Lo trae ~61 % de los leads. El ~39 % restante NO tiene campaña y se reporta por cómo llegó: "Meta pagado sin campaña" (pauta pagada — \`sessionSource: "Paid Social"\`— a la que Meta no le pasó el parámetro; desde agosto 2026 es el 100 % de los leads: hay que corregirlo en los anuncios, no en el CRM), "Meta orgánico / mensaje directo" (\`sessionSource: "Social media"\`), importación csv / captura manual, y otro origen. No repartas esos leads entre campañas. \`campaign\` trae "creativo / campaña".
